# MiniMax T2A HTTP — verbatim OpenAPI spec
...
> directly.
>
> The original page title is 同步语音合成 HTTP (synchronous TTS). The
> companion English doc lives in `MINIMAX-T2A-FINDINGS.md`; this file is
> the verbatim reference when you need to look up a field's exact name,
> range, or default. Section numbers in the findings doc are the
> canonical mapping from "field I saw in the response" to "what the API
> actually meant".

...
| `PI-TELEGRAM-TTS-PLAN.md` | Forward-looking build plan for the `pi-telegram-tts` sister extension (v0.1.0 → v0.6.0 SHIPPED; v0.7.0 is future expansion), and the inventory of upstream surfaces we have access to for future expansion. The plan's `## Progress` table + the per-version Acceptance matrices are the live source of truth. v0.1.0 / v0.2.0 / v0.3.0 / v0.4.0 / v0.6.0 are SHIPPED; v0.7.0 is the next planned (temp-file cleanup + in-Telegram commands + status line). |

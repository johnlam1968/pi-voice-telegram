...
| **v0.2.0** — Section UI | ⏳ next | — | Unblocked: `loadSynthConfig()` already reads on every call (live edit takes effect on next voice-tagged turn); only needs `saveSynthConfig` (atomic temp+rename, mirror `pi-telegram-stt/telegram-config.ts:71-96`) + `section.ts` mirroring `pi-telegram-stt/echo-section.ts:26-114`. |

...
| **v0.7.0** — Temp-file cleanup + in-Telegram commands + status line | pending | — | Future expansion. |
...
| §6.3 (design doc) spawn via absolute path on dev, by name when npm-installed | both resolution strategies work | (synth.ts `resolveScriptPath()` — manual code review; no separate test) | covered (deferred to v0.7.0) |
...
| Temp-file cleanup | `setTimeout(unlink(ogg), 30_000)` scheduled after bridge's `uploadVoiceFile` | — | **v0.7.0** |
| In-Telegram commands (`/tts_status`, `/tts_test`) | diagnostic surface | — | **v0.7.0** |
| Status line | row in `/start` menu's compact status | — | **v0.7.0** |
...
| In-Telegram commands (`/tts_status`, `/tts_test`) | diagnostic surface | — | **v0.7.0** |
| Temp-file cleanup (`setTimeout(unlink(ogg), 30_000)`) | OGG removed after upload | — | **v0.7.0** |
| Status line (row in `/start` menu) | compact status | — | **v0.7.0** |
...
| Temp OGG cleanup (`setTimeout(unlink, 30_000)`) | OGG removed after bridge upload | — | **v0.7.0** |
| In-Telegram commands (`/tts_status`, `/tts_test`) | diagnostic surface | — | **v0.7.0** |
| Status line in `/start` menu | compact status row | — | **v0.7.0** |

...
| **v0.2.0** — Section UI | draft | — | Plan drafted; see `### v0.2.0 — Section UI` below. Adds `section.ts` + `saveSynthConfig` + `startConfigWatcher` + 6 smoke stages. Mirrors `pi-telegram-stt/echo-section.ts` + `telegram-config.ts:174-202` + `index.ts:282-387`. |
...
| **v0.5.0** — Temp-file cleanup + in-Telegram commands + status line | pending | — | Future expansion. |
...
| In-Telegram commands (`/tts_status`, `/tts_test`) | diagnostic surface | — | **v0.5.0** |
| Temp-file cleanup (`setTimeout(unlink(ogg), 30_000)`) | OGG removed after upload | — | **v0.5.0** |
| Status line (row in `/start` menu) | compact status | — | **v0.5.0** |
...
v0.3.0 → v0.5.0. v0.4.0 will mark them `> Deprecated: use the
...
| Temp OGG cleanup (`setTimeout(unlink, 30_000)`) | OGG removed after bridge upload | — | **v0.5.0** |
| In-Telegram commands (`/tts_status`, `/tts_test`) | diagnostic surface | — | **v0.5.0** |
| Status line in `/start` menu | compact status row | — | **v0.5.0** |

...
|---|---|---|---|
| 2026-08-23 | `voice.sendTranscript` is not exposed in any Telegram UI (no main-menu toggle, no setter API). Operators can only edit the field by manually editing `telegram.json`. **Upstream bridge gap, not a regression** — the field is read by the synthesis provider (`getTelegramVoiceSendTranscript()`) but the bridge doesn't ship a setter or main-menu UI for it. | **Closed by removal (upstream v0.38.0, 2026-08-24)** | [llblab/pi-telegram#235](https://github.com/llblab/pi-telegram/issues/235) is moot — the bridge's v0.38.0 changelog (line "Coherent Voice Reply Policy") removed the field, the helper, and the provider-returned `transcriptText`. Synthesis providers now return only the OGG path; "text + voice" is the agent's explicit composition. The original issue framing ("add a UI toggle") was met with "this whole concept is redundant; remove it" instead. The v0.4.0 work in `docs/PI-TELEGRAM-TTS-PLAN.md` §v0.4.0 is the migration. |
...
8. **Upstream issue (now closed)**: Originally filed [llblab/pi-telegram#235](https://github.com/llblab/pi-telegram/issues/235) for the `voice.sendTranscript` UI gap. Closed on 2026-08-24 when the operator upgraded to v0.39.1: upstream v0.38.0 removed the entire feature rather than adding a setter (changelog line "Coherent Voice Reply Policy"). The v0.4.0 migration in `docs/PI-TELEGRAM-TTS-PLAN.md` §v0.4.0 is the response.

...
# `pi-telegram-stt` v0.10.0 sister extension (which now bundles
# the OpenAI-compatible STT provider and has dropped the section
# UI). No agent, no bridge, no Telegram — just jiti-load the
# package source and exercise the in-process STT provider
# registry + config merge, with one optional stage that drives a
# live STT round-trip.
#
# ## What it tests
#
...
# 7. No Section UI (v0.10.0) — `section.ts` and `section.js` are
#    GONE from the package, and `index.ts` does not import them.
#    Mirrors the v0.4.0 tts smoke's stage 16 (the operator dropped
#    the section UI on 2026-08-24 because `telegram.json`-driven
#    config is sufficient for single-operator setups).
#
...
# ---------------------------------------------------------------------------
hr
echo "pi-telegram-stt-smoke-test: stage 1/7 — jiti load + module-load registration"
...
// Load all source files; module-load side effects run.
// v0.10.0: section.ts was dropped (operator directive 2026-08-24).
for (const f of ["index.ts", "echo-handler.ts", "telegram-config.ts", "stt-provider.ts", "openai-stt.ts", "_logger.ts"]) {
...
# ---------------------------------------------------------------------------
hr
echo "pi-telegram-stt-smoke-test: stage 2/7 — re-load idempotency"
...
# ---------------------------------------------------------------------------
hr
echo "pi-telegram-stt-smoke-test: stage 3/7 — unconfigured → provider callable"
...
# ---------------------------------------------------------------------------
hr
echo "pi-telegram-stt-smoke-test: stage 4/7 — config merge (flat v0.8.0 shape)"
...
# ---------------------------------------------------------------------------
hr
echo "pi-telegram-stt-smoke-test: stage 5/7 — invalid base_url → network error"
...
if [[ $NO_NETWORK -eq 1 ]]; then
  echo "pi-telegram-stt-smoke-test: stage 6/7 — live STT round-trip (skipped: --no-network)"
  info "re-run without --no-network to exercise the full transcribe path against base_url"
else
  echo "pi-telegram-stt-smoke-test: stage 6/7 — live STT round-trip"
...
# ---------------------------------------------------------------------------
# 7. No Section UI (v0.10.0) — section.ts/section.js are GONE,
#    and index.ts does not import them. Mirrors the v0.4.0 tts
#    smoke's stage 16 (the section file was dropped per the
#    operator's 2026-08-24 directive because telegram.json-driven
#    config is sufficient for single-operator setups).
# ---------------------------------------------------------------------------
hr
echo "pi-telegram-stt-smoke-test: stage 7/7 — v0.10.0 no Section UI (the section file was dropped)"
...
console.log("section.ts/section.js are gone (per operator request 2026-08-24: drop the UI for stt completely)");
...
echo "  network:  $([ $NO_NETWORK -eq 1 ] && echo "skipped (stage 6 not run (6 is the only live network stage))" || echo "exercised (stage 6 ran (the only live network stage))")"
echo "  section:  dropped in v0.10.0 (stage 7 verifies the file is gone + no import in index.ts)"
echo ""

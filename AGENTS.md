...
1. **Two sister extensions** under `extensions/`:
   ...
   - **`pi-telegram-tts`** (v0.1.0, shipped 2026-08-23; v0.2.0 subsumes the runtime scripts; v0.3.0 adds per-provider sub-blocks + Draft 2020-12 schema; v0.4.0 adds `composeWithText: "auto"` text+voice composition; v0.6.0 drops the in-package `saveSynthConfig` writer + `loadTelegramConfig` reader — same design rule as the stt v0.11.0 drop) — TTS synthesis provider. Spawns the bundled `tts-*.mjs` scripts (subsumed from the deprecated `pi-voice-telegram-scripts` package) and reuses the same `ffmpeg` wrap. The package's `bin` field exposes `tts-minimax` and `tts-openai` on PATH after `npm install`. The section UI was **dropped on 2026-08-24** per the operator's directive; UI-driven config is no longer planned.
...
- `extensions/pi-telegram-tts/` — sister extension: TTS synthesis provider (v0.6.0; v0.1.0 shipped 2026-08-23, v0.2.0 subsumes the runtime scripts, v0.3.0 adds per-provider sub-blocks, v0.4.0 adds the `composeWithText: "auto"` text+voice composition, v0.6.0 drops the in-package `saveSynthConfig` writer + `loadTelegramConfig` reader — same design rule as the stt v0.11.0 drop). The v0.2.0 + v0.4.0 stage-2 `/telegram-settings` section UI was **dropped on 2026-08-24** at the operator's request — the `telegram.json`-driven config is sufficient. Self-contained (bundles its own `_logger.ts`). Publishable as `pi-telegram-tts` to the npm registry. See `docs/PI-TELEGRAM-TTS-PLAN.md` for the full version history + deferred work. The bundled scripts (`tts-minimax.mjs`, `tts-openai.mjs`) live in the same dir; the package's `bin` field exposes them on PATH after `npm install`. `fw-openai-sts.ts` is dropped (replaced by the system service).
...
- `scripts/pi-telegram-tts-smoke-test.sh` — 16-stage v0.4.0/v0.6.0 acceptance test for the `pi-telegram-tts` sister extension (jiti load + idempotency + 3 fall-through paths + live TTS round-trip + 4 `getVoicePromptContribution` shapes + bridge callable contract + bundled-scripts check + 3 v0.3.0 sub-block tests + "no Section UI" regression test; `--no-network` skips stage 6 for CI).
...
- `scripts/pi-telegram-tts-smoke-test.sh` — 16-stage acceptance test for the `pi-telegram-tts` sister extension: 1-5 module-load + fall-throughs, 6 live round-trip (network), 7-10 `getVoicePromptContribution` × 4 reachable shapes (manual/mirror/always × text/voice), 11 bridge callable contract, 12 bundled scripts, 13-15 v0.3.0 sub-block tests, 16 "no Section UI" regression test (the section file was dropped on 2026-08-24 per the operator's request). Run this after any change to `extensions/pi-telegram-tts/*.ts`. Use `--no-network` for CI / offline runs (skips stage 6 only).

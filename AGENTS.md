...
- **`dev/pi-telegram-stt`** — future work: the LLM tool surface (`synthesize_voice`, `transcribe_audio`, `pi_voice_telegram_*` config tools), the voice-catalog tool, and the `pi-voice-telegram-settings` extension (UI section for editing `telegram.json` on the fly). The v0.3.0 → v0.5.0 `pi-telegram-tts` work (per-provider sub-blocks, schema, UI-driven config) has been absorbed: per-provider sub-blocks + the Draft 2020-12 schema are on master as of v0.3.0; the section UI + UI-driven config were dropped on 2026-08-24 per the operator's directive (the agent can edit `telegram.json` directly via its existing tool surface). The LLM tools + companion-config schema are on this branch — they don't belong on master (per the v0.19.0 split).
...
**Live-test observation (2026-08-23)**: `voice.sendTranscript` is not exposed in any Telegram UI (no main-menu toggle, no setter API in the upstream bridge). This is a pre-existing bridge gap, not a regression from the consolidation. **Filed as upstream issue** [llblab/pi-telegram#235](https://github.com/llblab/pi-telegram/issues/235). Deferred until the bridge ships a setter API or main-menu UI. Workaround (if needed before then): an extension can write `voice.sendTranscript` directly to `telegram.json` via a small file-write helper, atomic temp+rename (same pattern as `saveEchoConfig`); the bridge's config hot-reload picks it up.

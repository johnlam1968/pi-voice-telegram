...
1. **Two sister extensions** under `extensions/`:
   - **`pi-telegram-stt`** (was `pi-telegram-echo` before v0.18.1) — STT orchestrator: registers the voice transcription provider, looks up the configured STT provider in the in-process registry, sends the 🎙️ echo to the user, feeds the transcript into the agent prompt. **As of v0.8.0, the OpenAI-compatible STT provider is bundled inside this package** (previously a separate `pi-openai-stt` npm package, now deprecated). The `SttProvider` interface stays as a private in-package seam for future backends. **As of v0.10.0, the `/telegram-settings` section UI was removed** (operator's 2026-08-24 directive; matches the v0.4.0 tts section drop) — all configuration is via `telegram.json#extensions["pi-telegram-stt"]` with a 200ms hot-reload watcher. **As of v0.11.0, the in-package `saveEchoConfig` atomic-write helper was dropped** (operator's design rule: every config knob lives in `telegram.json`, edited by the operator or agent via filesystem tools, picked up live by the hot-reload watcher). The in-package `loadEchoConfig` reader stays because it's the extension's own config interface at call time.
   - **`pi-telegram-tts`** (v0.1.0, shipped 2026-08-23; v0.2.0 subsumes the runtime scripts; v0.3.0 adds per-provider sub-blocks + Draft 2020-12 schema; v0.4.0 adds `composeWithText: "auto"` text+voice composition; v0.6.0 drops the in-package `saveSynthConfig` writer + `loadTelegramConfig` reader; v0.7.0 drops the bundled `tts-*.mjs` scripts + the per-provider sub-block + the `schema.json` + the `bin` field — direct `fetch` to the configured provider with a hardcoded `MINIMAX_BODY` / `OPENAI_BODY` constant) — TTS synthesis provider. The LLM's reply is the only field interpolated at call time; all other API params are constants in `synth.ts`. The section UI was **dropped on 2026-08-24** per the operator's directive; UI-driven config is no longer planned.
...
3. **Dev tools** (also under `scripts/`):
   ...
   - **`pi-telegram-tts-smoke-test.sh`** — 16-stage v0.4.0/v0.6.0 acceptance test for the `pi-telegram-tts` sister extension (jiti load + idempotency + 3 fall-through paths + live TTS round-trip + 4 `getVoicePromptContribution` shapes + bridge callable contract + bundled-scripts check + 3 v0.3.0 sub-block tests + "no Section UI" regression test; `--no-network` skips the network stage for CI).
...
## Latest session — 4 → 2 active npm package consolidation (2026-08-23)

**SHIPPED in a single session (2026-08-23):** the active npm-package count went from 4 to 2. Three superseded packages are now deprecated on npm. The repo lost 2 extension directories and gained 1 new smoke test.

### End state

| Package | Status | Version | Notes |
|---|---|---|---|
| `pi-telegram-stt` | ✅ active | 0.8.1 | STT orchestrator + bundled OpenAI-compatible STT provider (subsumed from `pi-openai-stt` in v0.8.0) |
| `pi-telegram-tts` | ✅ active | 0.2.0 | TTS synthesis provider + bundled `tts-{minimax,openai}.mjs` scripts (subsumed from `pi-voice-telegram-scripts` in v0.2.0) |
| `pi-openai-stt` | ⚠️ deprecated | 0.3.2 | Subsumed into `pi-telegram-stt@0.8.0`; `npm install pi-openai-stt` prints a deprecation warning |
| `pi-voice-telegram-scripts` | ⚠️ deprecated | 0.1.2 | Subsumed into `pi-telegram-tts@0.2.0`; `npm install pi-voice-telegram-scripts` prints a deprecation warning |
| `pi-voice-telegram` | ⚠️ deprecated | 0.16.12 | Old monolithic (split in v0.19.0); deprecated in v0.21.0 |

### Phase-by-phase execution

| Phase | Goal | Result |
|---|---|---|
| **Phase 1** (v0.21.0) | Deprecate `pi-voice-telegram@0.16.12` | ✅ Done via the npm web UI (default message, documented gap; CLI auth paths are all blocked) |
| **Phase 2** (v0.22.0) | Subsume `pi-openai-stt` into `pi-telegram-stt@0.8.0`; new 6-stage smoke test | ✅ Done; `pi-telegram-stt@0.8.0` + `pi-telegram-stt@0.8.1` published to npm; `pi-openai-stt@0.3.2` deprecated on npm |
| **Phase 3** (v0.23.0) | Merge `pi-voice-telegram-scripts` into `pi-telegram-tts@0.2.0`; new smoke stage 14 | ✅ Done; `pi-telegram-tts@0.2.0` published to npm; `pi-voice-telegram-scripts@0.1.2` deprecated on npm |
| **Phase 4** | Live test on the host (`@pimon_on_host_2026bot`) | ✅ Done after the user restarted `pi`; new pipeline fires end-to-end (4+ voice messages, 3+ TTS OGG outputs); first attempt was a transient first-run issue |
| **Phase 5** | Documentation (AGENTS.md, READMEs, plans) | ✅ Done; v0.21.0 + v0.22.0 + v0.23.0 changelogs; "Migration from 0.7.2" + "Migration from 0.1.2" sections; updated PI-TELEGRAM-TTS-PLAN + UPSTREAM-API-COMPLIANCE + CONSOLIDATION-PLAN |
| **Phase 6** | Verifier | ✅ Done; PARTIAL verdict returned, gaps fixed post-verifier (missing v0.23.0 changelog section + stale README install instructions) |

### Smoke tests (all green)

- `scripts/pi-telegram-stt-smoke-test.sh` — **6/6 stages** (new, v0.8.0). Stage 6 is a live STT round-trip against the local `whisper-server`.
- `scripts/pi-telegram-tts-smoke-test.sh` — **14/14 stages** (was 13, +1 for stage 14 in v0.2.0: bundled scripts + bin field check). Stages 6/7/8 are live TTS round-trips with `sendTranscript: true` / `false`.

### Documented gaps

1. **Deprecation messages** are npm's generic default ("Package no longer supported..."), not the custom text from `docs/CONSOLIDATION-PLAN.md` Appendix B. The npm web UI's "Deprecate package" button doesn't accept a custom message; the CLI auth paths (OIDC trusted publishing only covers `npm publish`; GAT requires interactive 2FA) are all blocked. The maintainer accepts this as a documented gap.
2. **`voice.sendTranscript` UI** — **closed by upstream** in `@llblab/pi-telegram@0.38.0` (file `CHANGELOG.md` line "Coherent Voice Reply Policy"). The bridge removed the entire feature rather than adding a setter: `voice.sendTranscript` config, `getTelegramVoiceSendTranscript()` helper, and the `transcriptText` field on the synthesis provider return are all gone. Synthesis providers now return only the OGG path; "text + voice" is the agent's explicit composition, not an automatic policy. **The original issue #235 framing ("add a UI toggle") is no longer the right ask** — the upstream's answer was "this whole concept is redundant; remove it." Both extensions need to be updated: see the v0.4.0 work tracked in `docs/PI-TELEGRAM-TTS-PLAN.md`.
3. **`mmx-tts-smoke-test.sh:213`** references `$(dirname "$0")/tts-minimax.mjs` which doesn't exist (the v0.19.0 split + the v0.2.0 merge both moved the script). Pre-existing bug; not blocking.
4. **Live test first-run failure** at 19:52:32 EDT (transient race condition during the new pi's first session); subsequent attempts all succeeded. Documented in `docs/CONSOLIDATION-PLAN.md` Phase 4 "Live-test observations" table.
...
- The full plan: [`docs/CONSOLIDATION-PLAN.md`](docs/CONSOLIDATION-PLAN.md)
- The per-phase changelogs (this file): v0.21.0 (Phase 1), v0.22.0 (Phase 2), v0.23.0 (Phase 3)
- The plan's Progress table: [`docs/PI-TELEGRAM-TTS-PLAN.md`](docs/PI-TELEGRAM-TTS-PLAN.md) (latest rows: `pi-telegram-stt@0.8.0` ✅, `pi-telegram-tts@0.2.0` ✅, Repo-wide consolidation ✅)
- The verifier's report: [`/tmp/consolidation-verification-report.md`](/tmp/consolidation-verification-report.md) (PARTIAL verdict; gaps fixed)
- The upstream issue: [llblab/pi-telegram#235](https://github.com/llblab/pi-telegram/issues/235)
...
- **`dev/pi-telegram-stt`** — future work: the LLM tool surface (`synthesize_voice`, `transcribe_audio`, `pi_voice_telegram_*` config tools), the voice-catalog tool, and the `pi-voice-telegram-settings` extension (UI section for editing `telegram.json` on the fly). The v0.3.0 → v0.6.0 `pi-telegram-tts` work (per-provider sub-blocks, schema, config-writer drop) has been absorbed: per-provider sub-blocks + the Draft 2020-12 schema are on master as of v0.3.0; the config writer (and its section-UI parent) were dropped on 2026-08-24 per the operator's directive (the agent can edit `telegram.json` directly via its existing tool surface). The LLM tools + companion-config schema are on this branch — they don't belong on master (per the v0.19.0 split).
...
- `extensions/pi-telegram-stt/` — sister extension: inbound STT + 🎙️ echo (renamed from `pi-telegram-echo` in v0.18.1). v0.10.0 (2026-08-24) — section UI removed (operator directive); v0.9.0 (2026-08-24) — section rename + dead code removal + flow refactor; v0.8.0 — bundles the OpenAI-compatible STT provider (previously a separate `pi-openai-stt` package). Self-contained (bundles its own `_logger.ts` since v0.7.0). Publishable as `pi-telegram-stt` to the npm registry.
- `extensions/pi-telegram-tts/` — sister extension: TTS synthesis provider (v0.7.0; v0.1.0 shipped 2026-08-23, v0.2.0 subsumes the runtime scripts, v0.3.0 adds per-provider sub-blocks, v0.4.0 adds the `composeWithText: "auto"` text+voice composition, v0.6.0 drops the in-package `saveSynthConfig` writer + `loadTelegramConfig` reader, v0.7.0 drops the bundled `tts-*.mjs` scripts + the per-provider sub-block + the `schema.json` + the `bin` field — direct `fetch` to the configured provider with a hardcoded `MINIMAX_BODY` / `OPENAI_BODY` constant in `synth.ts`). The v0.2.0 + v0.4.0 stage-2 `/telegram-settings` section UI was **dropped on 2026-08-24** at the operator's request — the `telegram.json`-driven config is sufficient. Self-contained (bundles its own `_logger.ts`). Publishable as `pi-telegram-tts` to the npm registry. See `docs/PI-TELEGRAM-TTS-PLAN.md` for the full version history + deferred work. The `bin` field is gone (v0.7.0); the scripts are gone; the schema is gone. The provider is the sole TTS path. `fw-openai-sts.ts` is dropped (replaced by the system service).
...
- `scripts/pi-telegram-stt-smoke-test.sh` — 7-stage v0.10.0 acceptance test for the `pi-telegram-stt` sister extension (jiti load + idempotency + unconfigured fall-through + config merge + invalid base_url + optional live STT round-trip + "no Section UI" regression test; `--no-network` skips stage 6 for CI).
- `scripts/pi-telegram-tts-smoke-test.sh` — 15-stage v0.7.0 acceptance test for the `pi-telegram-tts` sister extension (jiti load + idempotency + 3 fall-through paths + live TTS round-trip + 4 `getVoicePromptContribution` shapes + bridge callable contract + 3-field `SynthConfig` shape + hardcoded `MINIMAX_BODY` sanity check + "no bundled scripts" regression test + "no Section UI" regression test; `--no-network` skips stage 6 for CI).
...
- `scripts/pi-telegram-tts-smoke-test.sh` — 15-stage acceptance test for the `pi-telegram-tts` sister extension: 1-5 module-load + fall-throughs, 6 live round-trip (network), 7-10 `getVoicePromptContribution` × 4 reachable shapes (manual/mirror/always × text/voice), 11 bridge callable contract, 12 v0.7.0 SynthConfig shape (no voice/model/sub-block), 13 v0.7.0 hardcoded MINIMAX_BODY sanity check, 14 v0.7.0 no bundled scripts regression, 15 v0.4.0 "no Section UI" regression. Run this after any change to `extensions/pi-telegram-tts/*.ts`. Use `--no-network` for CI / offline runs (skips stage 6 only).
...
> **⚠️ Docker-container boundary.** A `pi` process visible in `ps` / `top` may be a Docker container (e.g. `pi-agent-jane`) — **not your on-host agent**. The on-host `pi` is the one you start yourself from an interactive terminal with `pi 2>/tmp/pi.stderr.log`; the bridge routes that one to your Telegram bot. **Never kill a `pi` process for "being in the way" without first checking `pstree -p <pid>`** — a Docker container's `pi` is nested under `containerd-shim` or `docker-proxy`; your on-host `pi` is nested under your shell (`bash` / `zsh` / the parent that ran `pi`). The same boundary applies to the bridge polling daemon: it's harmless to leave running; restarting it can wait for the next `pi` start. **For development: only touch the on-host `pi`** — leave Docker containers and the polling daemon alone unless you know they belong to your work. (Captured 2026-08-24: the live test recipe led an agent to mis-identify a Docker container as the on-host agent and almost kill it; the container was running other unrelated work.)
...
The Acceptance matrix for v0.1.0 has 31 rows; 26 map to a smoke stage, 5 are explicit deferrals to v0.2.0 / v0.3.0 / v0.4.0 / v0.6.0. See `### v0.1.0 — Acceptance matrix` in the plan doc for the full table.
...
- **v0.4.0** — UI-driven config (form-based edits from the Telegram section; later dropped in the v0.6.0 config-writer trim per the operator's 2026-08-24 directive).
- **v0.6.0** — drop the in-package config writer (`saveSynthConfig` + `loadTelegramConfig`); the agent's `read`/`write` tools are the canonical surface.
- **v0.7.0** — temp-file cleanup + in-Telegram commands (`/tts_status`, `/tts_test`) + status line.
...
✅ **All 3 consolidation phases shipped.** End state: 2 active packages (`pi-telegram-stt@0.8.0` + `pi-telegram-tts@0.2.0`) + 3 deprecated. See the v0.22.0 and v0.23.0 changelogs below for the per-phase details + operator-facing migration steps.
...
- `extensions/pi-telegram-stt/telegram-config.ts` — `EchoConfig` gains `base_url?: string | string[]` and `apiKey?: string` (v0.8.0 flat shape). The reader still accepts the legacy `extensions["pi-openai-stt"].base_url` / `.api_key` block for backward compatibility (read-only fallback). `saveEchoConfig` only writes the new flat shape. **The `saveEchoConfig` writer was dropped in v0.11.0** (operator's design rule: every config knob lives in `telegram.json`, edited by the operator or agent via filesystem tools). The `loadEchoConfig` reader stays.
...
**Live test** (Phase 4) — DONE on 2026-08-23 after the user restarted `pi` to load the new source. The new pipeline (STT in `pi-telegram-stt` → agent → TTS in `pi-telegram-tts` with bundled scripts → voice reply with caption) fires correctly: 4+ voice messages received, 3+ TTS OGG outputs created. The first attempt at 19:52:32 failed with `every voice synthesis provider failed` (transient first-run issue, likely a race condition during the new pi's first session); subsequent attempts all succeeded. **Smoke tests had already verified the new code end-to-end** before the live test: 6/6 stages of `scripts/pi-telegram-stt-smoke-test.sh` pass (including the live STT round-trip against the local `whisper-server`); 14/14 stages of `scripts/pi-telegram-tts-smoke-test.sh` pass.

**Live-test observation (2026-08-23)**: `voice.sendTranscript` is not exposed in any Telegram UI (no main-menu toggle, no setter API in the upstream bridge). This was a pre-existing bridge gap, not a regression from the consolidation. **Filed as upstream issue** [llblab/pi-telegram#235](https://github.com/llblab/pi-telegram/issues/235). **Closed by upstream** in `@llblab/pi-telegram@0.38.0` — the bridge removed the entire feature rather than adding a setter API (changelog line "Coherent Voice Reply Policy"). The `voice.sendTranscript` config, the `getTelegramVoiceSendTranscript()` helper, and the `transcriptText` field on the synthesis provider return are all gone. The v0.4.0 tts `composeWithText: "auto"` config re-implements the v0.1.0 "voice with caption" UX as two adjacent messages (text first, then voice) via the agent's explicit composition — not as a single voice message with a caption.

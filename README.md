...
3. **LLM tool surface (v0.6.0+, opt-in)** — when `tools.enabled: true` in the companion settings file, registers four additional tools the agent can call explicitly:
   ...
   - `pi_voice_telegram_config_read` / `pi_voice_telegram_config_write` (v0.11.0+, **double opt-in**: `tools.enabled: true` AND `tools.writable: true`) — let the LLM read the current settings and modify them via schema-validated atomic writes. The write tool refuses to modify `$schema`, `_hint`, or any key not in the schema. Default: NOT registered (operator deny-by-default). When registered, the promptGuidelines instruct the LLM to read first then write, and to tell the operator to restart the session after a write.
...
| `pi.registerTool` (from `@earendil-works/pi-coding-agent`) — `pi_voice_telegram_config_read` (v0.11.0+, double opt-in) | Reads the current companion settings (full file or per-key via dotted path). Refuses nothing — read is non-destructive. |
| `pi.registerTool` (from `@earendil-works/pi-coding-agent`) — `pi_voice_telegram_config_write` (v0.11.0+, double opt-in) | Schema-validated atomic write of a single key. Refuses `$schema`, `_hint`, and any unknown key. Returns old → new diff. The LLM is told to remind the operator to restart the session. |

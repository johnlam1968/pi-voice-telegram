...
3. **LLM tool surface (v0.6.0+, opt-in)** — when `llm_tools.exposed: true` in the companion settings file, registers four additional tools the agent can call explicitly:
...
| `llm_tools.exposed` | `false` | Master switch for LLM tool registration. When `false`, none of the 7 LLM tools (synthesize_voice, transcribe_audio, pi_voice_telegram_schema, pi_voice_telegram_config_read / _write / _reset, pi_voice_telegram_list_voices) are registered. When `true`, they're all available. v0.16.9+: renamed from `tools.enabled` to `tools.exposed`. v0.16.10+: namespace renamed from `tools` to `llm_tools` (the `llm_tools.` prefix makes it explicit that these switches gate the LLM tool surface, not the TTS/STT extension features). |
| `llm_tools.tts.enabled` | `true` | Register `synthesize_voice`. Honored only when `llm_tools.exposed` is `true`. |
| `llm_tools.tts.name` | `synthesize_voice` | Override the tool name (rare; for namespace collision avoidance). The description / promptSnippet / promptGuidelines text is templated against the resolved name (v0.8.0+). |
| `llm_tools.stt.enabled` | `true` | Register `transcribe_audio`. Honored only when `llm_tools.exposed` is `true`. |
| `llm_tools.stt.name` | `transcribe_audio` | Override the tool name. Templated like `llm_tools.tts.name`. |
